namespace FluentValidation.Tests
{
	...
	using System;
	[TestFixture]
	public class ChainedSharedConditionTests
	{
		private class ChainedSharedConditionValidator : AbstractValidator<Person>
		{
			public ChainedSharedConditionValidator()
					{
		...
		private class ChainedSharedConditionWithScopedUnlessValidator : AbstractValidator<Person>
		{
			public ChainedSharedConditionWithScopedUnlessValidator()
					{
						...
						// RuleFor() respects the outer When() and 
						// Unless() predicates.
						// 
						When( x => x.Id > 0 ).Unless(x => x.Age > 65)
							.RuleFor( x => x.Forename ).NotEmpty() )
							.RuleFor( x => x.Orders.Count ).Equal( 0 ).Unless( x => String.IsNullOrWhiteSpace( x.CreditCard ) == false ) )
							.RuleFor( x => x.AnotherInt ).GreaterThan(0) );
					}
		}
		...
		{
			var validator = new ChainedSharedConditionValidator();
		...
		{
			var validator = new ChainedSharedConditionWithScopedUnlessValidator();
			var person = new Person()
			{
				Id       = 4						// triggers the shared When predicate
			};
